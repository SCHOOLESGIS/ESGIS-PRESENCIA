...
<script setup>
    ...
    const {getAllRapportsArchived, deleteRapport} = useRapport()
    const data = useState("rapportsDataArchive")
    const links = useState("rapportLinksArchive")
    ...
    watch(first, function() {
        ...
        getAllRapportsArchived(page)
    })



    onMounted(() => {
        getAllRapportsArchived(0)
    });
