...
<script setup>
    ...
    const {getAllEmargementsArchived, deleteEmargement} = useEmargement()
    const data = useState("emargementsDataArchived")
    const links = useState("emargementLinksArchived")
    ...
    watch(first, function() {
        ...
        getAllEmargementsArchived(page)
    })



    onMounted(() => {
        getAllEmargementsArchived(0)
    });
